#### Dissimilarity: Model x inventary ####
...
dir.create("data/ModelsxInventary")
...
#Import PAM sourced from inventary
inv <- read.csv("data/Inventario_SC/PAM_SC_04jul24.csv")

#Import info on forest patch area
invdata <- fread("data/Inventario_SC/Tamanho Unidades Amostrais IFFSC.csv",
                 encoding = "Latin-1")

#Get raster base of Santa Catarina
r <- rast("Rasters/base.tiff")
plot(r)

#Get list of PAMS
pam_list <- list.files(path = "data/PAM/", full.names = TRUE)
#Read
pams <- pblapply(pam_list, fread)
#Names
names(pams) <- pam_list %>% fs::path_ext_remove() %>% basename()

#### Compare - Looping 2.5 arc-min ####
...
#Save
fwrite(all_res, "data/ModelsxInventary/Results.gz", row.names = FALSE,
       compress = "gzip")
...
#Import results
res <- fread("data/ModelsxInventary/Results.gz") %>% as.data.frame()
...
row.names(s) <- NULL
#Save results
fwrite(s, file = "data/ModelsxInventary/Summary_Results.gz",
